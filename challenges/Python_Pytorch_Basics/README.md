...
You can open these images on Summit by doing:
...
> Note: You can only open the images if you connected to Summit with window forwarding enabled. This is usually done by including the `X` or `Y` SSH flags when connecting to the system. For example: `ssh -XY USER_ID@login1.summit.olcf.ornl.gov`.
